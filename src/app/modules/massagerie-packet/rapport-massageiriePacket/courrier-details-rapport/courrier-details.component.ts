...
ngOnInit(): void {
    ...
    this.iduser= Number(this.sessionService.getAgentAttributes()?.id)
    this.getCourriersByFermetureIdAndStatut(this.fermetureId)
    this.getFermetureById(this.fermetureId)
}
    getCourriersByFermetureIdAndStatut(fermetureId:number ){
        this.fermetureCourrierService.getFermetureCourriersByFermetureId(fermetureId).subscribe(
            (result) => {

...
import type { CreateUpdatePoidsDto, PoidsDto } from './models';
...
export class PoidsService {
  ...
  findAll()
{
  return this.httpClient.get<[PoidsDto]>(this.api_host,this.httpOptions);
}
...
update(id:string, item:PoidsDto)
{
  let new_api_host = this.routerParam(this.api_host,id);
  return this.httpClient.put<PoidsDto>(new_api_host,item,this.httpOptions);
}

getOneById(id:string)
{
  let new_api_host = this.routerParam(this.api_host,id);
  return this.httpClient.get<PoidsDto>(new_api_host,this.httpOptions);
}
